...
*/
class Administracion_usuariosController extends Administracion_mainController
{
	/**
	 ...
	 * @var string
	 */
	protected $_csrf_section = "administracion_usuarios";
	...
	{
		$this->mainModel = new Administracion_Model_DbTable_Usuarios();
	...
	private function getData()
	{
		...
		$data['identificacion'] = $this->_getSanitizedParam("identificacion");
		$data['dv'] = $this->_getSanitizedParam("dv");
		$data['email'] = $this->_getSanitizedParam("email");
		$data['contacto_nombre'] = $this->_getSanitizedParam("contacto_nombre");
		$data['contacto_email'] = $this->_getSanitizedParam("contacto_email");
		$data['contacto_cargo'] = $this->_getSanitizedParam("contacto_cargo");
		$data['contacto_nombre_2'] = $this->_getSanitizedParam("contacto_nombre_2");
		$data['contacto_email_2'] = $this->_getSanitizedParam("contacto_email_2");
		$data['contacto_cargo_2'] = $this->_getSanitizedParam("contacto_cargo_2");
		return $data;
	}
	/**
    ...
    protected function getFilter()
    {
        ...
        if (Session::getInstance()->get($this->namefilter)!="") {
            ...
            if ($filters->identificacion != '') {
                $filtros = $filtros." AND identificacion LIKE '%".$filters->identificacion."%'";
            }
            if ($filters->dv != '') {
                $filtros = $filtros." AND dv LIKE '%".$filters->dv."%'";
            }
            if ($filters->email != '') {
    ...
    protected function filters()
    {
        if ($this->getRequest()->isPost()== true) {
            ...
            $parramsfilter = array();
					...
					$parramsfilter['identificacion'] =  $this->_getSanitizedParam("identificacion");
					$parramsfilter['dv'] =  $this->_getSanitizedParam("dv");
					$parramsfilter['email'] =  $this->_getSanitizedParam("email");Session::getInstance()->set($this->namefilter, $parramsfilter);
        }
