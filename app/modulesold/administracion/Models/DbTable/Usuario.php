...
class Administracion_Model_DbTable_Usuario extends Db_Table
{
	...
	public function insert($data)
	{
		...
		$uid = strtoupper(dechex(mt_rand(1000, 9999)) . str_replace('.', '', uniqid('', true)));

		$user_uid = $uid;
		$query = "INSERT INTO user(user_state, user_date, user_names, user_cedula, user_email, user_telefono, user_level, user_user, user_password, user_delete, user_current_user, user_code, user_empresa, user_uid) VALUES ( '$user_state', '$user_date', '$user_names', '$user_cedula', '$user_email', '$user_telefono', '$user_level', '$user_user', '$user_password', '$user_delete', '$user_current_user', '$user_code', '$user_empresa', '$user_uid')";
	...
	public function editFieldById($id, $field, $value)
	{
		$query = ' UPDATE ' . $this->_name . ' SET ' . $field . ' = "' . $value . '" WHERE user_id = "' . $id . '"';
		$res = $this->_conn->query($query);
	}
}
