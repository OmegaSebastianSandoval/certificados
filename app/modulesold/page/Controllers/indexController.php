...
class Page_indexController extends Page_mainController
{
  ...
  public function registrarAction()
  {
    ...
    if (!$user) {
      ...
      $dataSolicitud['email'] = APPLICATION_ENV != "production" ? $datosProveedor['emailProveedor'] : $datosProveedor['emailProveedor'];
      ...
      $userData['email'] = APPLICATION_ENV != "production" ? $datosProveedor['emailProveedor'] : $datosProveedor['emailProveedor'];
  ...
  public function enviarRecuperacionAction()
  {
    ...
    if ($user) {
      ...
      if (!$user->email) {
        $response = [
          'status' => 'error',
          'message' => 'El usuario no tiene un correo electrónico válido registrado.'
        ];
        die(json_encode($response));
      }
...
<soapenv:Envelope 
  ...
  public function actualizarcorreosAction()
  {
    $this->setLayout('blanco');
    $usersModel = new Administracion_Model_DbTable_Usuarios();
    $proveedoresModel = new Administracion_Model_DbTable_Proveedores();
    ...
    foreach ($users as $user) {
      ...
      $proveedorActualizado = $proveedoresModel->getList("proveedor_nit = '$nit' AND proveedor_emailProveedor = '$email'", "proveedor_id DESC")[0];
      if (!$proveedorActualizado) {
        $proveedorConsulta = $proveedoresModel->getList("proveedor_nit = '$nit'", "proveedor_id DESC")[0];
        if ($proveedorConsulta) {
          $newEmail = $proveedorConsulta->proveedor_emailProveedor;
          $usersModel->editField($user->id, 'email', $newEmail);
          echo "Usuario ID {$user->id} email actualizado a {$newEmail}<br>";
          $updatedCount++;
          $changes[] = [
            'usuario_id' => $user->id,
            'nit' => $nit,
            'email_anterior' => $email,
    ...
    // Registrar log
    $data['log_tipo'] = "ACTUALIZAR_CORREOS";
    $data['log_usuario'] = 'SYSTEM';
    $data['log_log'] = "Actualización de correos electrónicos completada. Usuarios actualizados: {$updatedCount}. Cambios: " . json_encode($changes, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    $logModel = new Administracion_Model_DbTable_Log();
    $logModel->insert($data);
