			...
			<div class="row">
				<div class="col-3">
					...
					<label class="input-group">
						...
						<input type="text" class="form-control" name="log_usuario" value="<?php echo $this->getObjectVariable($this->filters, 'log_usuario') ?>"></input>
					</label>
				</div>
				<div class="col-3">
					<label>tipo</label>
					<label class="input-group">
						...
						<select class="form-control" name="log_tipo">
							...
							<?php foreach ($this->list_log_tipo as $key => $value) : ?>
								<option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'log_tipo') ==  $key) {
																								echo "selected";
																							} ?>><?= $value; ?></option>
							<?php endforeach ?>
						</select>
			...
			<table class=" table table-striped  table-hover table-administrator text-left">
				...
				<tbody>
					<?php foreach ($this->lists as $content) { ?>
						...
						<tr class="texto-log">
							<td><?= $content->log_usuario; ?></td>
							<td><?= $this->list_log_tipo[$content->log_tipo]; ?>
							<td><?= $content->log_fecha; ?></td>
							<td><?= $content->log_log; ?></td>
							<td class="text-right">
								<div>
									<a class="btn btn-azul btn-sm d-none" href="<?php echo $this->route; ?>/manage?id=<?= $id ?>" data-bs-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pen-alt"></i></a>
									<span data-bs-toggle="tooltip" data-placement="top" title="Eliminar"><a class="btn btn-rojo btn-sm d-none" data-bs-toggle="modal" data-bs-target="#modal<?= $id ?>"><i class="fas fa-trash-alt"></i></a></span>
								</div>
								<!-- Modal -->
