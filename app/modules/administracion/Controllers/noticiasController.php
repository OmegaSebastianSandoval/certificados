...
class Administracion_noticiasController extends Administracion_mainController
{
	...
	{
		$data = array();
		if($this->_getSanitizedParam("noticia_fecha") == '' ) {
			$data['noticia_fecha'] = '0';
		} else {
			$data['noticia_fecha'] = $this->_getSanitizedParam("noticia_fecha");
		}
		if($this->_getSanitizedParam("noticia_titulo") == '' ) {
			$data['noticia_titulo'] = '0';
		} else {
    ...
    protected function filters()
    {
        if ($this->getRequest()->isPost()== true) {
            ...
            $parramsfilter = array();
					...
					$parramsfilter['noticia_titulo'] =  $this->_getSanitizedParam("noticia_titulo");Session::getInstance()->set($this->namefilter, $parramsfilter);
        }
